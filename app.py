...
def update_table(data):
    if not data:
        ...
        main_data.drop(['iFrame','CNT'], axis=1, inplace=True)
...
def update_graph(data,view):
    if not data:
        ...
        data_segment=[]
        clusters=[]
        colors=['#00BFB2','#0094C6','#CF1259','#9FFFCB','#D99AC5','#F7A072','#044B7F']
    ...
    if n_clicks!=None:
        if not data:
            ...
            predict_save_columns = ['Sample', 'N-AccX', 'N-AccY', 'N-AccZ', 'N-GyroX', 'N-GyroY', 'N-GyroZ', 'T-AccX', 'T-AccY',
                                    'T-AccZ', 'T-GyroX', 'T-GyroY', 'T-GyroZ', 'Information', 'Label', 'Predictions']
